...
document.querySelectorAll('.add-to-cart').forEach(button => {
    button.onclick = () => {
        const product = button.closest('.product');
        const isOutOfStock = product.classList.contains('out-of-stock'); // Controlla se è esaurito

        if (isOutOfStock) {
            alert('Questo articolo è esaurito e non può essere aggiunto al carrello.');
            return; // Esce dalla funzione se l'articolo è esaurito
        }
